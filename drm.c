...
static int create_imported_buffer(int drm_fd, int import_fd, struct gem_buffer *buffer)
{
	uint32_t handle;
	unsigned int i;
	int rc;

	rc = drmPrimeFDToHandle(drm_fd, import_fd, &handle);
	if (rc < 0) {
...
int display_engine_start(int drm_fd, unsigned int crtc_id, unsigned int plane_id, unsigned int width, unsigned int height, struct video_buffer *video_buffers, unsigned int count, struct gem_buffer **buffers, struct display_setup *setup)
{
	...
	*buffers = malloc(count * sizeof(**buffers));
	...
	for (i = 0; i < count; i++) {
		...
		if (use_dmabuf)
			rc = create_imported_buffer(drm_fd, video_buffer->export_fd, buffer);
		else
